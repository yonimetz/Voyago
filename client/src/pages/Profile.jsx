...
import { User, Shield, Sparkles, Mail, AlertTriangle, Trash2, Edit2, ArrowLeft, ArrowRight, Plus, Globe, Key, X, Lock, Save } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-sans pb-20" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <main className="max-w-3xl mx-auto px-6 py-12">
        <h1 className="text-4xl font-bold tracking-wide text-slate-900 mb-8">
        ...
        <div className="space-y-8">
          ...
          {/* כרטיסיית פרטי חשבון */}
          <div className="bg-white p-8 rounded-[2rem] shadow-[0_4px_24px_rgba(0,0,0,0.02)] border border-slate-200">
            <h2 className="text-xl font-bold text-slate-800 mb-6 flex items-center gap-2">
              <Shield className="w-5 h-5 text-[#0770E8]" /> {t('account_details', 'Account & Security')}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">{t('username_label', 'Username')}</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    className={`w-full p-4 bg-slate-50 hover:bg-white text-slate-800 border border-slate-200 focus:border-[#0770E8] focus:ring-4 focus:ring-[#0770E8]/10 rounded-2xl outline-none transition-all text-sm font-medium ${isRTL ? 'pr-11' : 'pl-11'}`}
                  />
                </div>
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">{t('email_label', 'Email')}</label>
                <div className="relative">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-5 mt-2 border-t border-slate-100 items-center justify-between">
              <div className="flex items-center gap-4 w-full sm:w-auto">
                 <Globe className="w-4 h-4 text-slate-400" />
                 <select 
                   value={i18n.language} 
                   onChange={handleLanguageChange}
                   className="bg-slate-50 hover:bg-white border border-slate-200 text-slate-700 font-medium text-sm rounded-xl focus:ring-2 focus:ring-[#0770E8]/20 focus:border-[#0770E8] block w-full p-2.5 outline-none cursor-pointer transition-all"
                 >
              ...
              <button 
                ...
                className="w-full sm:w-auto px-5 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold border border-slate-200/60 rounded-xl transition-all flex items-center justify-center gap-2 text-xs uppercase tracking-wider"
              >
                <Key className="w-4 h-4 text-slate-400" /> {t('change_password_btn', 'Change Password')}
              </button>
            </div>
          </div>

          {/* כרטיסיית העדפות חכמות */}
          <div className="bg-gradient-to-br from-blue-50/40 to-slate-50/50 p-8 rounded-[2rem] shadow-[0_4px_24px_rgba(0,0,0,0.01)] border border-blue-100/60">
            <h2 className="text-xl font-bold text-slate-800 mb-2 flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-[#0770E8]" /> {t('smart_profile', 'Smart Travel Profile')}
            </h2>
            ...
            <div className="mb-6">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">{t('suggested_prefs', 'Suggested Preferences:')}</p>
              <div className="flex flex-wrap gap-2">
                {suggestedPreferences.map((suggestion, idx) => (
                  <button 
                    ...
                    className="bg-white hover:bg-[#0770E8] hover:text-white text-[#0770E8] border border-blue-100 text-xs font-semibold px-4 py-2 rounded-full transition-colors flex items-center gap-1 shadow-sm"
                  >
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-6">
              <input 
                ...
                className="flex-1 p-4 bg-white border border-blue-100 rounded-2xl focus:ring-2 focus:ring-[#0770E8]/40 focus:border-[#0770E8] outline-none text-sm font-medium transition-all"
              />
              <button 
                type="button"
                onClick={() => handleAddPreference()}
                className="bg-[#0770E8] hover:bg-blue-700 text-white px-8 py-4 rounded-2xl font-bold text-sm tracking-wide transition-all shadow-sm active:scale-95 disabled:bg-slate-300 flex items-center justify-center gap-2"
            ...
            {preferencesList.length > 0 && (
              <div className="bg-white rounded-2xl p-4 border border-blue-50 shadow-inner">
                ...
                <ul className="space-y-2">
                  {preferencesList.map((pref, index) => (
                    <li key={index} className="flex justify-between items-center p-3 bg-slate-50 border border-slate-100 rounded-xl hover:bg-slate-100 transition-colors group">
                      <span className="text-sm font-medium text-slate-700 mx-2">{pref}</span>
                      <div className="opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
                        <button onClick={() => handleEdit(index, pref)} className="text-[#0770E8] hover:bg-blue-50 p-2 rounded-lg transition-colors" title="Edit"><Edit2 className="w-4 h-4" /></button>
                        <button onClick={() => handleDelete(index)} className="text-red-400 hover:bg-red-50 p-2 rounded-lg transition-colors" title="Delete"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </li>
                  ))}
                </ul>
          ...
          <div className="flex justify-end pt-4 pb-8 border-b border-slate-200">
            <button 
              onClick={handleSave}
              disabled={isSaving}
              className="w-full md:w-auto bg-[#0770E8] hover:bg-blue-700 text-white px-8 py-4 rounded-2xl font-bold transition-all shadow-lg shadow-blue-200 transform active:scale-95 disabled:bg-slate-400 flex items-center justify-center gap-2 text-sm tracking-wide"
            >
              {isSaving ? <Sparkles className="w-4 h-4 animate-pulse" /> : <Save className="w-4 h-4" />}
              {isSaving ? t('saving', 'Saving...') : t('btn_save_profile', 'Save Profile Changes')}
            </button>
          </div>

          {/* אזור מסוכן */}
          <div className="bg-red-50/50 p-8 rounded-[2rem] border border-red-200/60 mt-12">
            ...
            <button 
              ...
              className="bg-white hover:bg-red-600 text-red-600 hover:text-white border border-red-100 hover:border-red-600 px-6 py-3 rounded-xl font-bold transition-all text-xs uppercase tracking-wider flex items-center gap-2 shadow-sm"
            >
      ...
      {isPasswordModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-[2.5rem] p-8 max-w-sm w-full shadow-2xl animate-fade-in">
            ...
            <div className="w-16 h-16 bg-blue-50 text-[#0770E8] rounded-2xl flex items-center justify-center mb-6 shadow-sm">
            ...
            <form onSubmit={handlePasswordSubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">{t('current_password', 'Current Password')}</label>
                <input 
                  type="password" 
                  required
                  value={passwordData.current}
                  onChange={(e) => setPasswordData({...passwordData, current: e.target.value})}
                  className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-[#0770E8]/50 focus:border-[#0770E8] outline-none transition-all text-sm font-medium"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">{t('new_password', 'New Password')}</label>
                <input 
                  type="password" 
                  required
              ...
              <div className="mb-2">
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">{t('confirm_password', 'Confirm New Password')}</label>
                <input 
                  type="password" 
                  required
              ...
              <div className="flex gap-3 pt-4 mt-6 border-t border-slate-100">
                <button 
                  type="button"
                  onClick={() => setIsPasswordModalOpen(false)}
                  className="flex-1 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors"
                >
                  {t('cancel', 'Cancel')}
                </button>
                <button 
                  ...
                  className="flex-1 py-3 text-xs font-bold uppercase tracking-wider text-white bg-[#0770E8] hover:bg-blue-700 rounded-xl transition-colors shadow-md shadow-blue-100 disabled:bg-slate-400"
                >
