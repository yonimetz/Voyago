...
import { ChevronDown, Map, Compass, Users, Globe, X, Mail } from 'lucide-react';
...
function RegisterPage({ setCurrentUser }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(t('register_error', 'Registration failed. Username or email might be taken.'));
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="relative z-10 max-w-7xl w-full mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="w-full max-w-md mx-auto bg-white/5 backdrop-blur-sm p-10 rounded-[2.5rem] shadow-2xl border border-white/20">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                <label className="block text-sm font-bold text-white mb-2 mx-1 drop-shadow-sm">{t('register_email', 'Email')}</label>
                <input 
                  type="email" 
                  value={email}
              ...
              >
                {isLoading ? t('register_connecting', 'Creating Account...') : t('register_btn', 'Create Account')}
              </button>
            </form>
            
            <p className="mt-8 text-center text-white/80 text-sm font-medium">
               {t('register_has_account', 'Already have an account?')} 
              <Link to="/login" className="mx-2 text-white font-bold hover:text-blue-300 hover:underline transition-colors">{t('register_login', 'Login here')}</Link>
            </p>
          </div>
        </div>
